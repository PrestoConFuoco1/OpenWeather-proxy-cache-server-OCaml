...
module OWParams = struct
  let rec asum_option opts =
    match opts with
    | [] -> None
    | o :: os -> ( match o with None -> asum_option os | Some _ as res -> res)
  ...
  let get_time (req : Request.t) =
    ...
    | None -> Unix.time () |> Int.of_float |> Option.some
  ...
  let usage =
    "Usage: ?lat=FLOAT&lon=FLOAT or ?name=STRING or ?zipcode=STRING; optional \
     dt=INT (default: now)"
end
...
module SharedCache : sig
  ...
  let with_cache (cache_ref : cache_ref) (action : Cache.t -> Cache.t Lwt.t) :
      unit Lwt.t =
...
module ThreadLogTags = struct
  ...
  let mk_thread_tags filler_id = Logs.Tag.(empty |> add thread_tag filler_id)
end

module OWCall = struct
  ...
  let call_current_weather_by_coords ~api_key ({lat; lon} : latlong) =
    ...
    let uri_with_query_params =
      ...
      in
      List.fold_left
        ~f:(fun uri (k, v) -> add_query_param uri (k, [v]))
        ~init:base_uri
        query_params
    in
...
module Handler = struct
  ...
  let handler ~api_key ~(cache_ref : SharedCache.cache_ref) req :
      Response.t Lwt.t =
    ...
    | Some (location_identifier, time) ->
        let%lwt latlong =
          PlaceResolver.identifier_to_latlong location_identifier
        in
        let opt_cached =
          Cache.find latlong time (SharedCache.get_cache cache_ref)
        in
        let%lwt res =
          match opt_cached with
          | None ->
              let%lwt () =
                Logs_lwt.info (fun m ->
                    m "No entry found in cache, calling openweather API" ~tags)
              in
              let%lwt res =
                OWCall.call_current_weather_by_coords ~api_key latlong
              in
              let entry = Cache.entry_of_res res in
              let%lwt () =
                SharedCache.with_cache cache_ref (fun old_cache ->
                    let new_cache = Cache.add_time_entry entry old_cache in
                    let%lwt () =
                      Logs_lwt.debug (fun m ->
                          m
                            "Cache:\n%!%a"
                            (Repr.pp_dump Cache.print_cache_t)
                            (Cache.to_print_cache new_cache)
                            ~tags)
                    in
          ...
          | Some entry ->
              ...
              Lwt.return (Cache.res_of_entry entry)
        in
...
module CacheFiller = struct
  ...
  type env = {
    ...
    location : latlong;
        (** Coordinates of location which this filler is responsible of *)
    cache_ref : SharedCache.cache_ref;  (** Shared cache with weather data *)
  }

  let run_filler_thread env =
    let tags =
      ThreadLogTags.mk_thread_tags ("thread-" ^ Int.to_string env.idx)
    in

    let log_exception e =
      Logs.err (fun m -> m "%s\n" @@ Stdlib.Printexc.to_string_default e)
    in

    let rec cache_filler_thread env () =
      let%lwt res =
        OWCall.call_current_weather_by_coords ~api_key:env.api_key env.location
      in
      let%lwt () =
        SharedCache.with_cache env.cache_ref (fun old_cache ->
            let%lwt () =
              Logs_lwt.debug (fun m ->
                  m "Response:\n%!%a" (Repr.pp_dump response_t) res ~tags)
            in
      ...
      let%lwt () =
        Lwt_unix.sleep
          (Float.of_int (Option.value_exn env.cfg.Config.update_period_sec))
      in
      cache_filler_thread env ()
    in
      ...
      let with_stamp h tags k fmt =
        ...
        let thread_id : string =
          Option.value
            ...
            (match tags with
            | None -> None
            | Some tags -> Logs.Tag.find ThreadLogTags.thread_tag tags)
        in
        Caml.(
          Fmt.kpf
            k
...
let run () =
  ...
  let cache_ref =
    ...
    @@ Cache.create
         ...
         ~distance_accuracy_km:(Option.value_exn config.distance_accuracy_km)
  in
  let locations =
    Lwt_main.run
      (Lwt_list.map_p PlaceResolver.identifier_to_latlong config.locations)
  in
  let filler_env idx loc =
    CacheFiller.{idx; cfg = config; api_key; location = loc; cache_ref}
  in
  let () =
    List.iteri locations ~f:(fun idx loc ->
        CacheFiller.run_filler_thread @@ filler_env idx loc)
  in
