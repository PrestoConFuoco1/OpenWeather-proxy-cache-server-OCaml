...
module GeoDistance = struct
  ...
  let haversine ({lat = lat1; lon = lon1} : latlong)
      ({lat = lat2; lon = lon2} : latlong) =
    let open Float in
...
type timeseries = (int * current) List.t [@@deriving repr]

type place_info = {latlong : latlong; timezone : string; timezone_offset : int}
[@@deriving repr]

type entry = {place_info : place_info; time : int; entry : current}
...
type print_cache = (latlong * int list) list [@@deriving repr]
...
let opt_min_by lst ~compare =
  if List.is_empty lst then None else List.min_elt lst ~compare

let find_by_minimal_proximity lst compare_proximity max_proximity
    get_abs_proximity =
  let matching_units =
    List.filter_map lst ~f:(fun x ->
        let prox = get_abs_proximity x in
        if compare_proximity prox max_proximity <= 0 then Some (x, prox)
        else None)
  in
  Option.map
    ~f:fst
    (opt_min_by matching_units ~compare:(fun u1 u2 ->
         compare_proximity (snd u1) (snd u2)))

let find searched_l searched_time t =
  let open Option.Let_syntax in
  let%bind place =
    find_by_minimal_proximity
      t.cache
      Float.compare
      (Float.of_int t.distance_accuracy_km)
      (fun (l, _tsr) -> GeoDistance.haversine searched_l l.latlong)
  in
  let%bind time_entry =
    find_by_minimal_proximity
      (snd place)
      Int.compare
      t.time_accuracy_sec
      (fun (dt, _te) -> abs (dt - searched_time))
  in
  return {place_info = fst place; time = fst time_entry; entry = snd time_entry}

let add_time_entry {place_info; time; entry} t =
  let upd_ref = ref 0 in
  let new_t =
    List.map t.cache ~f:(fun ((l, tsr) as elt) ->
        if
          Float.(
            GeoDistance.haversine l.latlong place_info.latlong
            <= Float.of_int t.distance_accuracy_km)
        then (
          Int.incr upd_ref ;
          (l, List.Assoc.add tsr ~equal:Int.equal time entry))
        else elt)
  in
  if !upd_ref = 0 then
    {t with cache = (place_info, List.return (time, entry)) :: t.cache}
